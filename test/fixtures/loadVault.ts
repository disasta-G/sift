...
import { MANIFEST_NAME, type ManifestExtra, type ManifestNote, type VaultManifest } from './generate';
...
export interface LoadedVault {
	...
	totalLength: number;
	/** The canvas and base files that were loaded, empty unless `extras` was asked for. */
	extras: readonly ManifestExtra[];
}
...
/** What {@link loadVault} may include beyond the notes. */
export interface LoadOptions {
	/**
	 * Also load the generated canvas and base files.
	 *
	 * Off by default, and deliberately so: `limit` counts NOTES, `markerCounts`
	 * counts notes, and every existing assertion is phrased in notes. A suite
	 * that wants the mixed vault asks for it, which also makes the benchmark's
	 * two passes — notes only, and notes plus the rest — comparable.
	 */
	extras?: boolean;
}

/**
 * Loads every `.md` file under `dir`, plus the other kinds when asked.
 *
 ...
 * and a fast unit test without regenerating. It does not truncate the extras:
 * they are a fixed fraction of the full vault, and a caller that wants them
 * wants the proportion they come in.
 */
export function loadVault(dir: string = DEFAULT_VAULT_DIR, limit?: number, options: LoadOptions = {}): LoadedVault {
	...
	if (options.extras === true && manifest !== null) {
		for (const extra of manifest.extras ?? []) {
			let content: string;
			try {
				content = readFileSync(join(absolute, extra.path), 'utf8');
			} catch {
				// Generated by an older fixture run that did not write this kind.
				continue;
			}
			files[extra.path] = { content, ctime: extra.ctime, mtime: extra.mtime };
			totalLength += content.length;
			extras.push(extra);
		}
	}

	return { dir: absolute, files, manifest, notesByPath, totalLength, extras };
}

/** `loadVault` plus `createFakeApp` — the one call an Indexer or Searcher test needs. */
export function loadFakeApp(
	dir: string = DEFAULT_VAULT_DIR,
	limit?: number,
	options: LoadOptions = {},
): { app: FakeApp; vault: LoadedVault } {
	const vault = loadVault(dir, limit, options);
